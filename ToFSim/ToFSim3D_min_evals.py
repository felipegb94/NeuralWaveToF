...
ambientIllumFactorMax = 5 # Ambient illumination 5 times stronger than source
...
for i in range(0,numSampleScenes):
	...
	ambientIllum = ambientIllumFactor
	readNoise = readNoiseVar*np.random.normal(0,1)
	...
	estimatedDepthMap = np.zeros((sceneRows,sceneCols))
	for currRow in range(0,sceneRows):
		for currCol in range(0,sceneRows):
			...
			estimatedDepthMap[currRow,currCol] = estimatedDepth 
	...
	np.savetxt("../AnalyticalScenes/" + sceneID + "_" + str(depthOffsets[i]) + "_EvalScenes.csv",estimatedDepthMap,delimiter=",")
	np.savetxt("../GroundTruthScenes/" + sceneID + "_" + str(depthOffsets[i]) + "_EvalScenes.csv",estimatedDepthMap,delimiter=",")
...
datasetFilename = datasetDirname + '_EvalScenes'
